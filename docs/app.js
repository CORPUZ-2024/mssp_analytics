...
async function boot() {
  try {
    ...
    ALL_ROWS = decodeCounties(counties);

    renderProvenance();
    bind();
    selectTab("a");
  } catch (err) {
    document.getElementById("provenance").className = "provenance error";
    document.getElementById("provenance").innerHTML =
      `<b>Could not load the committed data payload.</b> ${esc(err.message)}. `
      + "The payload lives in <code>docs/data/</code> and is regenerated by the "
      + "<code>Refresh CMS data</code> workflow.";
  }
}
